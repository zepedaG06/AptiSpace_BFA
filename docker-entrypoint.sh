...
APP_PORT="${PORT:-8080}"
APP_DIR="/usr/local/tomcat/webapps/AptiSpace_BFA"
...
if [ ! -d "$APP_DIR" ]; then
  mkdir -p "$APP_DIR"
  (cd "$APP_DIR" && jar -xf /usr/local/tomcat/webapps/AptiSpace_BFA.war)
  rm -f /usr/local/tomcat/webapps/AptiSpace_BFA.war
fi
